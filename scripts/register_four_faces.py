...
DEFAULT_IMAGE_4 = Path(r"data/imgs/4.png")
DEFAULT_IMAGE_5 = Path(r"data/imgs/5.png")
DEFAULT_IMAGE_6 = Path(r"data/imgs/6.png")
DEFAULT_IMAGE_7 = Path(r"data/imgs/7.png")
...
def build_subject_specs(args: argparse.Namespace) -> list[SubjectSpec]:
    return [
        SubjectSpec(args.subject4_id, args.subject4_name, Path(args.image4)),
        SubjectSpec(args.subject5_id, args.subject5_name, Path(args.image5)),
        SubjectSpec(args.subject6_id, args.subject6_name, Path(args.image6)),
        SubjectSpec(args.subject7_id, args.subject7_name, Path(args.image7)),
    ]
...
def validate_base_url(value: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise argparse.ArgumentTypeError("base URL must look like http://127.0.0.1:8000")
    return value.rstrip("/")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--subject4-id", default="qq_face_004", help="Subject ID for image 4.")
    parser.add_argument("--subject4-name", default="qq-face-4", help="Display name for image 4.")
    parser.add_argument("--image4", default=str(DEFAULT_IMAGE_4), help="Path to face image 4.")

    parser.add_argument("--subject5-id", default="qq_face_005", help="Subject ID for image 5.")
    parser.add_argument("--subject5-name", default="qq-face-5", help="Display name for image 5.")
    parser.add_argument("--image5", default=str(DEFAULT_IMAGE_5), help="Path to face image 5.")

    parser.add_argument("--subject6-id", default="qq_face_006", help="Subject ID for image 6.")
    parser.add_argument("--subject6-name", default="qq-face-6", help="Display name for image 6.")
    parser.add_argument("--image6", default=str(DEFAULT_IMAGE_6), help="Path to face image 6.")

    parser.add_argument("--subject7-id", default="qq_face_007", help="Subject ID for image 7.")
    parser.add_argument("--subject7-name", default="qq-face-7", help="Display name for image 7.")
    parser.add_argument("--image7", default=str(DEFAULT_IMAGE_7), help="Path to face image 7.")
